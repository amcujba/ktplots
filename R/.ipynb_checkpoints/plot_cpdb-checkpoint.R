...
plot_cpdb <- function(cell_type1, cell_type2, scdata, idents, means, pvals, max_size = 8,
    ...
    if (!is.null(gene.family) & is.null(genes)) {
        ...
        th17 <- grep("IL21|IL22|IL24|IL26|IL17A|IL17A|IL17F|IL17RA|IL10|RORC|RORA|STAT3|CCR4|CCR6|IL23RA|TGFB",
            means_mat$interacting_pair)
        treg <- grep("IL35|IL10|FOXP3|IL2RA|TGFB|LGALS9", means_mat$interacting_pair)
        ...
        notch <- grep("^DLL|NOTCH|JAG", means_mat$interacting_pair)
        vegf <- grep("VEGFB|VEGFA|VEGFC|VEGFD|VEGFR|FLT1|FLT4|KDR|PGF|NRP1|NRP2|SDC1", means_mat$interacting_pair)
        fgf <- grep("^FGF|FGFR", means_mat$interacting_pair)
        egf <- grep("^EGF|EGFR", means_mat$interacting_pair)
        tgfb <- grep("^TGFB|TGFBR", means_mat$interacting_pair)
        angiopoietin <- grep("^ANGPT1|ANGPT2|ANGPT4|ANGPTL3|ANGPTL2|ANGPTL3|ANGPTL4|ANGPTL5|ANGPTL6|ANGPTL7|ANGPTL8|TIE2", means_mat$interacting_pair)
        adhesion <- grep("^SEL|ICAM|VCAM|JAM|CAM|ESAM", means_mat$interacting_pair)
        ecm <- grep("^FN1|COL|a1b1", means_mat$interacting_pair)
        antigen_presentation <- grep("^CD74|HLA", means_mat$interacting_pair)
        bp_regulation_and_vasodilation <- grep("^Epinephrine|Prostaglandin", means_mat$interacting_pair)
        traficking <- grep ("^SORT1", means_mat$interacting_pair)
        query_group <- list(chemokines = chemokines, chemokine = chemokines, th1 = th1,
            ...
            costimulation = costimulatory, coinhibition = coinhibitory, niche = niche, notch = notch, vegf = vegf, 
            fgf = fgf, egf = egf, tgfb = tgfb, angiopoietin = angiopoietin, adhesion = adhesion, ecm = ecm, antigen_presentation = antigen_presentation, bp_regulation_and_vasodilation = bp_regulation_and_vasodilation, traficking = traficking)
